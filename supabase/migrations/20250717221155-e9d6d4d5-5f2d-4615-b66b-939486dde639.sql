...
CREATE TABLE public.clients (
  ...
  transactional_text_opt_in BOOLEAN DEFAULT false,
  agree_to_liability_waiver BOOLEAN DEFAULT false,
  pre_arketa_milestone_count INTEGER,
  first_seen DATE NOT NULL, -- This is your "created date" for sequence calculation
  last_seen DATE,
  tags TEXT,
  intro_period_start DATE, -- When they started intro period (may differ from first_seen)
  intro_period_active BOOLEAN DEFAULT true,
...
CREATE TABLE public.message_templates (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  sequence_day INTEGER NOT NULL, -- 0, 7, 10, 14, 28
...
CREATE TABLE public.message_log (
  ...
  external_message_id TEXT -- For tracking with external APIs
);
...
CREATE POLICY "Allow authenticated users to access clients" ON public.clients
...
CREATE INDEX idx_clients_intro_period ON public.clients(intro_period_active, intro_period_start, first_seen);
